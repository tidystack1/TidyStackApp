  ...
  const testPdfFetch = async () => {
    ...
    try {
      const response = await fetch("/api/test-pdf", {
        ...
        body: JSON.stringify({ id: recordId, formType: "petty-cash" }),
      });
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
          ...
          <p className="text-gray-600 mb-6">
            Test fetching and combining PDFs from Zoho CRM before setting up
            webhooks
          </p>
          ...
          {result && !error && (
            <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="text-sm text-green-700 space-y-1">
                ...
                <p>
                  📎 Attachments Found:{" "}
                  <strong>{result.attachmentCount || 0}</strong>
                </p>
                <p>
                  📄 Total Files Combined:{" "}
                  <strong>{result.pdfCount || 0}</strong>
                </p>
                {result.pdfFiles !== undefined &&
                  result.imageFiles !== undefined && (
                    <p className="text-xs">
                      ({result.pdfFiles} PDF{result.pdfFiles !== 1 ? "s" : ""},{" "}
                      {result.imageFiles} image
                      {result.imageFiles !== 1 ? "s" : ""})
                    </p>
                  )}
        ...
        {result && result.attachmentCount === 0 && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
            <h3 className="text-yellow-800 font-semibold mb-2">
              ⚠️ No Attachments Found
            </h3>
            <p className="text-yellow-700">
              The record exists but has no attachments. Make sure the record has
              PDF files attached in Zoho CRM.
            </p>
          </div>
        )}
      </div>
