...
const REPORTS_PDF_FIELD_ID = "sdea90e90f";
const REPORTS_GENERATED_AT_FIELD_ID = "s7034d7c18";
...
async function generateCommitteePdf(
  ...
  const drawDataRow = (row: CommitteeExpandedRow) => {
  ...
  const groups = new Map<string, CommitteeExpandedRow[]>();
...
export async function POST(req: Request) {
  try {
    ...
    const expandedRows: CommitteeExpandedRow[] = [];
    ...
    expandedRows.sort((a, b) => {
      const approved = a.approvedFor.localeCompare(b.approvedFor, "en", {
        sensitivity: "base",
      });
    ...
    const pdfBuffer = await generateCommitteePdf(expandedRows);
    const filename = `committee_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    return NextResponse.json(
      {
        message: "Committee PDF generated and uploaded to SmartSuite",
